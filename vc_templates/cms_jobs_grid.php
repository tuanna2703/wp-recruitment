<?php if (class_exists('JobBoard')) {
    ...
    if ($atts['layout'] == 'layout1') { ?>
        <div id="<?php echo esc_attr($atts['html_id']); ?>"
            class="jb-carousel-wrap cms-recent-jobs <?php echo esc_html($atts['job_style']); ?> <?php echo esc_html($atts['custom_class']); ?>">
            ...
            <div id="cms-jobs-wrapper">
                <div class="jb-jobs-body jobs-carousel">
                    <?php while ($content->have_posts()):
                        ...
                        <?php if ($i % 4 == 0) {
                            echo "<div class='jb-carousel-items'>";
                        } ?>
                ...
                <div class="jb-jobs-footer clearfix">
                    <a class="btn view-all-job"
                        href="<?php echo jb_page_permalink('jobs'); ?>"><?php echo esc_html__('All Jobs', 'wp-recruitment'); ?></a>
                </div>
            </div>
        </div>
    <?php } else { ?>
        <div id="<?php echo esc_attr($atts['html_id']); ?>"
            class="cms-jb-carousel-layout2 <?php echo esc_html($atts['custom_class']); ?>">
            ...
            <?php while ($content->have_posts()):
                ...
                <?php if (class_exists('JobBoard')) { ?>
                    <div class="cms-jb-item">
                        ...
                        <div class="loop-meta">
                            <?php if ($atts['hide_location'] != '1'): ?>
                                <div class="loop-locations"><?php jb_job_the_locations(); ?></div>
                            <?php endif; ?>
                            <?php if ($atts['hide_salary'] != '1'): ?>
                                <div class="loop-salary"><?php jb_job_the_salary(); ?></div>
                            <?php endif; ?>
                        </div>
                        <div class="loop-type"><?php jb_job_the_types(); ?></div>
                        <div class="loop-content"><?php echo wp_trim_words(strip_tags(strip_shortcodes(get_the_content())), 28); ?>
                        </div>
